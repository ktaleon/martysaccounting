...
app.post("/expenses", authorization, async (req, res) => {
  try {
    ...
    const expenses = await pool.query(
      `INSERT INTO expenses (ingredients, salary, utilities, user_id) VALUES ('${ingredients}', '${salary}', '${utilities}', '${req.user}') RETURNING *`
    );
...
app.get("/data", authorization, async (req, res) => {
  try {
    const productsold = await pool.query(
      `SELECT * FROM productsold INNER JOIN product ON product.id = productsold.product_id`
    );
...
app.get("/id", authorization, async (req, res) => {
  try {
    const user = await pool.query(
      `SELECT user_id FROM users WHERE user_id = '${req.user}'`
    );
...
app.delete("/deleteproductsold", authorization, async (req, res) => {
  try {
    const deleteproductsold = await pool.query(
      `DELETE FROM productsold WHERE order_of_sale = (SELECT max(order_of_sale) FROM productsold) RETURNING *`
    );
    res.json(deleteproductsold.rows[0]);
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Server Error");
  }
